...
class Obstacle():
	def __init__(self, width = 4, height = 1, r = 0.1, c = 0.1, threshold=0.01, 
	...
	def plotSpace(self):
		quad_1_x, quad_1_y = [0.925, 1.075, 1.075, 0.925, 0.925],[ 0.575, 0.575, 0.425, 0.425, 0.575]
		quad_2_x, quad_2_y = [1.925, 2.075, 2.075, 1.925, 1.925],[ 0.575, 0.575, 0.425, 0.425, 0.575]
		quad_3_x, quad_3_y = [1.425, 1.575, 1.575, 1.425, 1.425],[ 0.35, 0.35, 0.2, 0.2, 0.35]
		quad_4_x, quad_4_y = [1.425, 1.575, 1.575, 1.425, 1.425],[ 0.8, 0.8, 0.65, 0.65, 0.8]
		self.ax.plot(quad_1_x, quad_1_y)
		self.ax.plot(quad_2_x, quad_2_y)
		self.ax.plot(quad_3_x, quad_3_y)
		self.ax.plot(quad_4_x, quad_4_y)
		self.ax.set_xlim(0, 4)
		self.ax.set_ylim(0, 1)
		pass

	def checkObstcaleSpace(self):
		xx = np.arange(0,4,0.05)
	...
	def ObsCheck(self, i, j):
		if self.checkBoundary(i,j):
			return False
		elif self.checkInQuad(i, j, 0.925, 1.075, 0.575, 0.425):
			return False
		elif self.checkInQuad(i, j, 1.925, 2.075, 0.575, 0.425):
			return False
		elif self.checkInQuad(i, j, 1.425, 1.575, 0.35, 0.20):
			return False
		elif self.checkInQuad(i, j, 1.425, 1.575, 0.8, 0.65):
			return False
	...
	def checkBoundary(self,i,j):
		# print(i,j)
		if (i < (4 - self.r - self.c) and  
 			i > (-1.0 + self.r + self.c) and 
...
class pathFinder():
	def __init__(self, initial, goal, thetaStep = 30, stepSize = 1, goalThreshold = 0.1,
		width = 4, height = 1, threshold = 0.5, r = 0.1, c = 0.1, wheelLength = 0.038, 
		Ur=5,Ul=5, wheelRadius=2, dt=0.1, dtheta=0, weight=1, showExploration=0, showPath=1):
...
if __name__ == '__main__':
	...
	Parser.add_argument('--Start', default="[0, 0.5, 30]", help='Inital location')
	Parser.add_argument('--End', default="[3.25, 0.5, 0]", help='Goal location')
	Parser.add_argument('--RobotRadius', default=0.12, help='Robot radius')
	Parser.add_argument('--Clearance', default=0.02	, help='Clearance')
	...
	Parser.add_argument('--WheelLength', default=0.26, help='wheelbase')
	Parser.add_argument('--RPM', default="[6, 10]", help='RPM values')
	...
	solver = pathFinder(initial, goal, stepSize=StepSize,
		goalThreshold = GoalThreshold, width = 4, height = 1, threshold = Threshold,
...
# 	print(solver.trackIndex)
	...
	file = open("outputtry.txt", "w+")
	# file.write('ENPM661-2022 \n')
	# file.write('Team Members: Pulkit Mehta, Anirudh Krishnan Komaralingam \t section: \n\n\n')
	# file.write('Time_step = 1.0 second(s) \n')
	...
	for idx in solver.trackIndex:
		...
		rz = r*(ur-ul)/l
		w = rz * 2 * 3.14 / 60
		file.write(str(round(rz,5)) + ' ' + str(round(vx,5)))
		file.write('\n')
# 		print(ul,ur,vx,w)
	file.close()
